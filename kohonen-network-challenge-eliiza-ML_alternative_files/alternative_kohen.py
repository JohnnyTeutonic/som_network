...
class SOM_NUMPY():
    def __init__(self, neurons, dimensions, n_iter=1000, learning_rate=0.1):
        neighbourhood_radius = max(neurons)/2
        self.neurons = neurons
        self.dimensions = dimensions
        self.weights = np.random.randint(0, 255, size=(neurons[0], neurons[1], self.dimensions)) / 255
    ...
    def _assignLabels(self, samples):
        dimensions = self.weights.shape
        self.weights_ = self.weights.reshape(dimensions[0] * dimensions[1], dimensions[2])
        labels = []
    ...
    def _updateWeights(self, sample):
        ...
        # Fund BMU neuron
        distances = cdist(self.weights.reshape(dimensions[0]*dimensions[1], dimensions[2]), sample, metric='euclidean')
        distances = distances.reshape(dimensions[0], dimensions[1])
        ...
        distances = cdist(self.weights.reshape(dimensions[0] * dimensions[1], dimensions[2]), 
                          closestNeuron.reshape(1, dimensions[2]), metric='euclidean')
        # Arrange distances as sorted index
        distances = np.argsort(np.argsort(distances.reshape(dimensions[0] * dimensions[1])))
        distances = distances.reshape(dimensions[0], dimensions[1])
...
s = SOM_NUMPY(neurons=(10,10), dimensions=3, n_iter=1000, learning_rate=0.1)
s.train(samples)
